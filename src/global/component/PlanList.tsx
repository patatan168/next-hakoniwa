...
const PlanItem = ({
  ...
  const { plan_no, x, y, plan } = item;
  const { name } = getPlanDefine(plan);
  return (
    <div
      ref={setActivator}
...
const SortableItem = memo(
  function SortableItem({ isChange, item, turn }: sortedItemProps) {
    const {
      isDragging,
...
export const PlanList = memo(
  function PlanList({ planData, turn }: PlanListProps) {
