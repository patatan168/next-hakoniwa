...
function updateLoginFailCount(
  ...
  db.prepare<[number, string | null, string], void>(
    `UPDATE user SET login_fail_count = ?, locked_until = ? WHERE uuid = ?`
  ...
  if (valid.data !== null) {
    ...
    const user = db.client
      .prepare<
        string,
        userSchemaType
      >(`SELECT uuid, id, password, login_fail_count, locked_until FROM user WHERE id = ?`)
      .get(hashId);

    if (user !== undefined) {
      // ログイン失敗回数チェック
      let failCount = user.login_fail_count + 1;
      ...
      if (verify) {
        ...
        updateLoginFailCount(db.client, user.uuid, failCount, lockedUntil);
